...
class DeliveryAmountsDriver extends React.Component {
      ...
      <div className="col-md-6 col-xl-4 mg-t-10">
        <div className="card ht-100p">
          <div className="card-header d-flex align-items-center justify-content-between">
            <h6 className="mg-b-0">Delivery Amounts By Driver</h6>
                ...
                return (
                  <li className="list-group-item d-flex pd-sm-x-20" key={index}>
                    ...
                    <div className="mg-l-auto text-right">
                      <p className="tx-medium mg-b-0">₦{this.formatTotal(data.nmv)}</p>
                    </div>
                  </li>
                )
              })
          ...
          <div className="card-footer text-center tx-13">
            <a href="#" className="link-03">View All Transactions <i className="icon ion-md-arrow-down mg-l-5" /></a>
          </div>{/* card-footer */}
        </div>{/* card */}
      </div>
...
function mapStateToProps(state) {
  return {
    auth: state.auth,
    deliveryAmountDriverData: state.graphData.deliveryAmountDriverData,
  }
}
...
function matchDispatchToProps(dispatch) {
  return {
    onGetGraphData() {
      dispatch(graphActions.querydeliveryAmountsDriver())
    }
...
export default connect(mapStateToProps, matchDispatchToProps)(DeliveryAmountsDriver);